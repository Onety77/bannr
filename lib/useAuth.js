...
// How long a stored wallet session is worth betting a hop on. See the
// note at `usable` in startWalletDeeplink for why this exists and why
// the number is a judgement rather than a documented lifetime.
const SESSION_TRUST_MS = 6 * 60 * 60 * 1000;
...
export function useAuth() {
  ...
  const { pendingSign, linked } = dlState;
  ...
  useEffect(() => {
    const onStorage = (e) => {
      ...
      setWalletState({ pendingSign: null, error: null });
      setBusy(false);
      setError(null);
  ...
  // ══ ONE TAP FOR ANYONE WHO HAS CONNECTED BEFORE ══
  //
  // The wallet session survives in localStorage, so a returning
  // visitor does not need the connect hop at all — only the
  // signature. What stopped us skipping it is that signing needs a
  // nonce, fetching one is an await, and an await before the
  // navigation spends the tap.
  //
  // So the challenge is fetched AHEAD of the tap, by whichever
  // component is about to show the button. If it is there and fresh
  // when they tap, that is one app-hop instead of two. If it is not,
  // nothing breaks: the connect hop is still there and still works.
  const armed = useRef(null);

  const armWalletDeeplink = useCallback(async () => {
    const mods = dl.current;
    if (!mods) return;
    const live = mods.DeepLink.anySession();
    if (!live?.publicKey) return;
    // Still fresh for this address? Nothing to do.
    const a = armed.current;
    if (a && a.address === live.publicKey && Date.now() - a.at < 3 * 60_000) return;
    try {
      const c = await mods.flow.fetchChallengeFor(live.publicKey);
      armed.current = { address: live.publicKey, provider: live.provider, nonce: c.nonce, message: c.message, at: Date.now() };
    } catch {
      armed.current = null;
    }
  }, []);
  ...
  //
  // `payload` is whatever the intent needs on the far side of the hop —
  // for "buy" it is which pack, and it MUST be threaded through to
  // beginFlow. It used to be accepted by the caller and by beginFlow
  // and by nothing in between, so buying credits chose a pack, hopped
  // to Phantom, came back with payload null, failed to match the pack,
  // and never built the transaction. The second hop is a button that
  // only appears once the transaction exists, so the next tap started
  // the connect hop over: Phantom asking to connect, endlessly, and
  // never asking to confirm anything.
  const startWalletDeeplink = useCallback((intent, provider = "phantom", payload = null) => {
    const mods = dl.current;
    ...
    try {
      ...
      // A nonce dies at five minutes on the server, so anything older
      // than four here would hop out to the wallet only to be refused
      // on the way back. Falling through to connect costs one extra
      // approval and always works.
      //
      // ══ AND THE SESSION HAS TO BE WORTH TRYING TOO ══
      //
      // The freshness check above is about the CHALLENGE. It said
      // nothing about the session the challenge would be signed with,
      // and that session lives in localStorage indefinitely while the
      // wallet stops honouring it much sooner. So a visitor returning
      // the next day took the one-tap path with a dead session, got
      // refused, and was told the connection had gone stale — every
      // time, on the first attempt, with the second attempt working
      // because the failure had cleared the session on its way out.
      //
      // This path exists to save ONE hop. Taking it on a session that
      // is probably dead costs a wasted hop plus an alarming message,
      // so it is now only taken while the session is recent enough to
      // be a fair bet. The number is a judgement, not a documented
      // lifetime: Phantom does not publish one. Too low and returning
      // users lose a shortcut they could have had; too high and the
      // failure comes back. Six hours keeps it for the same day.
      const a = armed.current;
      const session = mods.DeepLink.anySession();
      const fresh = session && Date.now() - (session.connectedAt || 0) < SESSION_TRUST_MS;
      // The "buy" exception that used to head this list is gone with
      // the flow it protected: paying no longer comes through here at
      // all. Every remaining intent ends in a signature, which is
      // exactly what this shortcut is for.
      const usable =
        a && session && fresh &&
        a.address === session.publicKey &&
        Date.now() - a.at < 4 * 60_000;

      if (usable) {
        mods.flow.beginFlow(intent, a.provider, payload, { address: a.address, nonce: a.nonce });
        mods.DeepLink.signMessage(a.provider, a.message, "wallet");
        armed.current = null; // single use, like the nonce it holds
        setBusy(true);
        return true;
      }

      mods.flow.beginFlow(intent, provider, payload);
  ...
  // Paying has no hook here any more. It is a Solana Pay transfer
  // request built and opened by the credits page — the wallet makes
  // the transaction itself, so there is nothing to hand it and nothing
  // to wait for a redirect about. See lib/solanaPay.js.

  // Called once a flow is finished with. Ends it and tells the tab the
  // user started in, which is still open somewhere.
  ...
  return {
    ...
    startWalletDeeplink,
    // Called by whatever is about to render the wallet button. Fetches
    // the challenge ahead of the tap so a returning visitor signs in
    // with one app-hop instead of two. Safe to call repeatedly.
    armWalletDeeplink,
    ...
    cancelWalletDeeplink: () => { setWalletState({ pendingSign: null }); setBusy(false); },
    finishFlow,

...
export function useAuth() {
  ...
  //
  // Nothing is awaited: signMessage and connect navigate the page
  // away. What happens next happens in the resume effect below, on a
  // different page load.
  const startWalletDeeplink = useCallback(async (intent, provider = "phantom") => {
    setError(null);
    setBusy(true);
    try {
      const [{ default: DeepLink }, flow] = await Promise.all([
        import("@/lib/deeplink"),
        import("@/lib/walletFlow"),
      ]);
      flow.beginFlow(intent, provider);

      // A wallet already connected in a previous visit skips straight
      // to signing. Two app-hops is the cost of a first connection,
      // not a toll on every attempt.
      const live = DeepLink.restoreSession(provider);
      if (live?.publicKey) {
        await flow.requestSignature(DeepLink, provider, live.publicKey);
      } else {
        DeepLink.connect(provider);
      }
      return true;
    } catch (e) {
      const why = e?.message || "Couldn't open your wallet app.";
      setError(why);
      setBusy(false);
      return false;
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Hop one came back with an address: get a challenge and hop out
      // again. This navigates away, so nothing below it runs.
      if (result.type === "connect") {
        if (!pending) return; // a stray redirect, not something we asked for
        setBusy(true);
        try {
          await flow.requestSignature(DeepLink, result.provider, result.publicKey);
        } catch (e) {
          flow.endFlow();
          setBusy(false);
          setError(e?.message || "Couldn't start signing.");
        }
        return;
      }

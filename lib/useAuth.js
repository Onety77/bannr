...
export function useAuth() {
  ...
  // The buy flow's two waypoints: an address to build a transaction
  // for, and a broadcast signature to confirm.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (result.type === "connect") {
        ...
        // Paying needs no challenge — the transaction itself is the
        // proof, and the memo inside it names the account. Hand the
        // address to whoever is showing the packs and let them build
        // it; see app/credits.
      ...
      // A broadcast transaction. The wallet has already sent it; all
      // that is left is to confirm it on-chain, which is the credits
      // page's job because it owns the packs and the claim.
      if (result.type === "signature") {
        setPendingPay(null);
        setBusy(false);
        setPaid({ signature: result.signature, payload: pending?.payload || null });
        return;
      }
      ...
      if (result.type === "message") {
        ...
        try {
          ...
          flow.endFlow();
          // The tab this started in is still open and still showing
  ...
  return {
    ...
    cancelWalletDeeplink: () => { setPendingSign(null); setPendingPay(null); setBusy(false); },

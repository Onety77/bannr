...
/**
 * Who does this amount belong to, asked WITHOUT a session.
 *
 * ══ THIS IS WHAT LETS THE WEBHOOK ATTRIBUTE A PAYMENT ══
 *
 * Everything else here starts from an account: a signed-in browser
 * says who it is and we check the amount against that account's
 * reservations. The webhook has no browser and no session — it is told
 * only that some wallet sent some SOL to the treasury. Its old answer
 * was to look the SENDER up among registered wallets, which fails for
 * every first-time buyer, because buying no longer requires linking a
 * wallet at all.
 *
 * The amount index is global and keyed by lamports, so it answers the
 * question the other way round: this exact amount was reserved, and
 * this is whose it is.
 *
 * Two reads rather than one, deliberately. The index says which
 * account; the account's own record carries the QUOTE, and keeping the
 * deal in one place means the webhook and the browser cannot credit
 * the same payment differently.
 */
export async function intentForAmount(lamports, blockTimeMs) {
  const db = getAdminDb();
  if (!db || !(lamports > 0)) return null;
  const snap = await db.collection(AMOUNTS).doc(String(lamports)).get();
  if (!snap.exists) return null;
  const owner = snap.data();
  if (!owner?.accountId || !fresh(owner, Date.now())) return null;
  const entry = await matchIntent(owner.accountId, lamports, blockTimeMs);
  return entry ? { accountId: owner.accountId, entry } : null;
}

...
// ============================================================
export async function generatePfp(prompt, { image, size = 1024 } = {}) {
  if (!aiEnabled()) return null;
  if (!image) throw new Error("generatePfp: an image is required — none was provided.");
  return imagesEdit(prompt, [{ b64: image, name: "subject.jpg" }], {
    size: `${size}x${size}`,
  });
}

...
- Each concept: 50–90 words, concrete enough to execute. Name the genre, the shot, the light, the setting, and where the name sits.`,
  ...
  him: {
    ...
    palette: `- READ THE SUBJECT FIRST. What kind of presence does THIS thing actually have? A snarling animal, a serene one, a chunky comic one and a weary one all command a frame differently, and the staging has to serve the one in front of you. Presence is not a single flavour, and treating it as one is how this style gets boring. Say in the concept what quality you are staging for.
- THE GROUND COMES FROM THE SUBJECT, NOT FROM A MOOD SHELF. Derive it from what this subject is made of, coloured like, or would plausibly stand in — a woven thing implies fabric and warm fibre, a chrome thing implies cold reflection, a hellhound implies ash and ember-light. Reaching for a black void or grey fog without a reason rooted in the subject is the house cliché of this style and it is how every banner starts looking like every other. Across the ${"COUNT"} concepts, no two grounds may be from the same family — two voids, or a void and a near-black fog, count as the same ground and have failed.
- SMALL IS A POWER MOVE ONLY WHEN THE EYE LANDS ON IT INSTANTLY. At most ONE of the ${"COUNT"} concepts may make the subject small in a vast field, and that concept must guarantee the find: the subject sits at the point of highest contrast, silhouetted or lit so nothing competes with it, on a strip a few centimetres wide on a phone. A subject that has to be hunted for has not commanded the frame — it has disappeared from it, which is the one failure this style cannot survive.
- EACH CONCEPT LEADS WITH A DIFFERENT LEVER. Presence comes from a small set of tools and the ${"COUNT"} options must not all reach for the same one. The levers: enormous scale, the subject cropped by the frame so it cannot be contained; extreme negative space, the subject small in a vast field and still commanding it; placement in frame, sitting high with the ground falling away low beneath it; light alone, one hard source in near-darkness; atmosphere, fog or haze or dust doing the work; proximity, close enough to read the material it is made of.
- ALMOST NOTHING ELSE IS IN FRAME. No props, no crowds, no set dressing, no environment with things happening in it. If you catch yourself describing a scene rather than a subject and a ground, you have written the wrong style. The ground is one plane and one atmosphere — a field, a void, a wall, a horizon, weather.
- NAME THE LIGHT PRECISELY: where it comes from, how hard it is, its colour temperature, what it rims and what it leaves in shadow. "Dramatic lighting" is not an answer; "a single hard source low and behind, rimming the shoulders and leaving the face in shadow" is.
- THIS BANNER CARRIES NO TEXT AT ALL. Do not plan a name, logo, caption or label anywhere in it. The subject alone carries the banner.
- YOU HAVE ONE VIEW OF THIS SUBJECT, AND ONLY ONE. The client uploaded a single image and that is the only angle of it anyone has ever seen. Never ask for a different one — no low angle looking up at it, no over-the-shoulder, no three-quarter turn, no view from behind, no "seen from below". Every one of those requires the renderer to invent surfaces it has never been shown, and inventing is exactly where a subject stops being the client's subject and becomes a lookalike. On this style that is fatal, because the subject IS the entire banner and there is nothing else for the eye to forgive it with.
- IMPLY THE VIEWPOINT BY ARRANGING THE WORLD, NOT BY TURNING THE SUBJECT. This is the craft of it. A thing reads as towering when the horizon sits low and the ground falls away beneath it, when it is placed high in the frame, when the light rakes up from below, when scale cues around it are small — and none of that touches the subject itself. Compose the space so the viewer feels they are looking up. Do not rotate what you were given.
- THE SUBJECT STAYS ITSELF — proportions, colours, features, markings and character preserved exactly, at the angle it was supplied. You are staging it, not redrawing it.
- QUIET, NOT LOUD. Explosions, lightning, glowing eyes and energy effects are what people reach for when they cannot make presence any other way. Authority is calm.
- Each concept: 50–90 words. Name the quality of presence, the lever, the light, how the subject sits in the frame, and the ground. Describe the FRAMING — what is included and where it sits — never a camera move or a new angle on the subject.`,
  },

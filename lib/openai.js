...
- Each concept: 50–90 words, concrete enough to execute. Name the register in the first clause, then the composition — what fills the frame, what the subject is doing, where the name sits and how large, the palette. Concrete nouns, no vibes.`,
  ...
  design: {
    ...
    palette: `- WORK OUT WHAT THIS PROJECT ACTUALLY IS, AND SAY SO IN THE FIRST CLAUSE OF EVERY CONCEPT. Read the name, the description and the attached mark TOGETHER and decide what the thing does — indexes, swaps, lends, routes, launches, secures — and who it is for. Use what you actually know about the world: if the mark is a chain's or a company's and you recognise it, that is real information about this project and it belongs in the thinking. A concept that could be pitched to any other client in the category is a failed concept, and the way you avoid that is by knowing what this one is before you draw anything.
- THE IDEA COMES FROM WHAT IT DOES, NEVER FROM WHAT IT IS CALLED. A launchpad called Honey is not honey, honeycomb or syrup; one whose tagline says "locked" is not padlocks, keys or vaults; a serious company called Decay, Inferno or Skull is still a serious company and its banner is not rot, fire or bones. Illustrating the word is the first idea anyone has and almost always the weakest in the room — it describes the label instead of the company. A name may earn a colour or one quiet motif. It never earns the picture.
- THE ${"COUNT"} CONCEPTS MUST BE COMPOSITIONALLY UNLIKE EACH OTHER — different archetypes, not variations of one. Draw from, and go beyond: typography AS the artwork; the mark repeated as a pattern or system; a pictorial metaphor filling the frame as a designed ground; an editorial or print layout; a FLAT material study — paper, ink, card, fabric, a printed surface seen face-on; a diagram or geometric system that IS the concept.
- NAME THE GROUND, AND LET THE IDEA CHOOSE IT. A photographed landscape, a real place, an object, a printed surface, a diagram, a pattern, a flat field — all are available, and which one is right is decided by the idea and never by a preference for emptiness. Ambition is welcome; atmosphere is welcome. What is not welcome is arbitrary furniture: everything in the frame earns its place by serving the one idea. AN EMPTY FRAME IS NOT RESTRAINT, IT IS AN ABSENT CONCEPT — a flat colour field with the mark and the name on it is the assembled default you exist to beat, and proposing it is the same failure as proposing clutter.
- BE LITERAL ABOUT WHAT THE PROJECT TOUCHES, and this is usually your strongest move. If it is about stocks, put the real companies in the frame — Apple, Tesla, Nvidia, by mark and by name. If it routes between chains, those chains are real and nameable. If it lends, the assets are real. You know these things; use them. "Various assets", "a technical-looking surface" and "abstract shapes representing value" are what you write when you have not had an idea, and a concept that names nothing real would suit any other project in the category.
- NONE OF THIS, however well you think you could execute one: the mark on one side with the name on the other over an empty field, which is the default you exist to beat; circuit boards, traces or PCB patterns in any role; hexagon grids, wireframe globes, floating code, binary digits, terminal windows, drifting interface panels; glowing particle fields, data streams, network-node constellations; invented corporate architecture wearing the client's logo; ornate machinery, gears, brass, engraved metal, keys, vaults or steampunk apparatus; fantasy or sci-fi environments and cityscapes. A crypto or infrastructure brief is the one that attracts all of it, which is exactly why the list exists. A real, recognisable place is welcome when the project genuinely points at it.
- THE MARK IS ATTACHED, AND IT TELLS YOU WHAT THIS IS — NEVER WHERE IT GOES. Whatever it was exported onto is not the brief; the ground is yours to invent and may CONTRAST with it rather than echo it. Choose what the mark is doing from this list and name your choice: placed small against a large field; repeated as a pattern; cropped hard by an edge; used as the counter of a letterform; the single element on a plain ground; left out because the name carries it. It stays a MARK — flat artwork placed on a design, never melted into a scene, reflected, refracted, embedded in an object or reduced to "its colour appearing somewhere". You can see it, so name its real hues and build a palette that works with them; take HUE, never FINISH, and keep to two or three colours and one accent.
- THE TYPE IS SET, NOT MADE. The project name appears in every concept, whole and readable, and no letter of it is ever sliced by the edge of the frame however deliberate that would look — it may be enormous or small and quiet, but a banner whose job is to identify a project cannot omit its name. Words from the tagline may carry the artwork, alongside the name and never instead of it. Two decorations are equally fatal: distress (flames, cracks, drips, rust, glitching) and polish (metallic, chrome or gold fills, bevel, emboss, drop shadow, outer glow, extrusion, 3D, gradients poured down the letters, reflections) — and never engraved, etched, embossed, carved or cut INTO anything, which is the same failure arriving as a manufacturing process. Describe how it is SET: weight, scale, case, tracking, alignment, how much room it is given, what it sits on.
- Each concept: 50–90 words, concrete enough to execute. What the project is, then the idea in one sentence, then the composition — the ground, what fills the frame, where the name sits and how large, what the mark is doing, the palette. Concrete nouns, no vibes.`,
  },
...
export async function generateConcepts({ brief, styleBrief, count, constraints = "", perConcept = [], director = "design", logo = null, mustObey = "" }, retried = false) {
...
Return strict JSON: {"concepts": ["...", "..."]} with exactly ${count} strings. No other text.`;
  ...
  try {
    ...
    if (out.length === 0 && !retried) {
      ...
      return generateConcepts({ brief, styleBrief, count, constraints, perConcept, director, logo, mustObey }, true);
    }

...
export async function generateBackground(prompt, { logo, refs = [], styleRefs = [], reimagine = false, restyle = "", mark = false } = {}) {
  ...
  // ══ STYLE REFERENCES: TAKE THE DESIGN, NEVER THE CONTENT ══
  //
  // THIS BLOCK WAS INVERTED AND IT COST US SIXTEEN UNUSABLE BANNERS.
  //
  // It used to forbid copying "layout, composition, subject matter,
  // colour palette, LIGHTING, overall darkness or brightness, mood,
  // typeface" and add that a banner resembling a reference "in any of
  // those respects has failed" — then ask, in the abstract, for "the
  // STANDARD".
  //
  // Every reference in references/tech is flat, light, restrained and
  // typographic. So the instruction read, correctly, as: do not be
  // restrained, do not be light, do not be typographic. The output was
  // dark ornate maximalism every single time, and Tek — the ONLY style
  // with references — became the worst style in the product. The
  // model was obeying.
  //
  // "The standard" is also not transmissible. A model cannot extract
  // craft as an abstraction; what it can read off a picture is exactly
  // the list that was being forbidden.
  //
  // So the line moves. The old fear — that showing an example gets it
  // copied — was real, but it is about CONTENT: the subject, the
  // scene, the words. Those stay forbidden. Everything about HOW the
  // thing is designed is now explicitly asked for, because that is the
  // entire reason a reference is attached.
  if (styleRefs.length > 0) {
    const n = styleRefs.length;
    const one = n === 1;
    finalPrompt +=
      ` The final ${one ? "attached image is a STYLE REFERENCE" : `${n} attached images are STYLE REFERENCES`}. ${one ? "It is" : "They are"} not this project.

NOTHING ${one ? "IT DEPICTS" : "THEY DEPICT"} belongs in your banner: not the subject matter, not the objects, places or scenes shown, not any word, wordmark or logo ${one ? "it carries" : "they carry"}. Never reproduce what ${one ? "it is a picture" : "they are pictures"} OF.

THE SPECIFIC FAILURE, BECAUSE IT HAS ALREADY HAPPENED TWICE: a logo or symbol was lifted out of a reference and drawn into the finished banner — another company's mark, sitting on this client's artwork. That is the worst outcome this instruction exists to prevent, worse than an ugly banner, because it ships someone else's brand as though it were the client's. The ONLY mark permitted anywhere in the frame is the client's own, attached as the first image. If a shape in a reference reads as a logo, an icon, an emblem or a monogram, it is not a shape you may borrow — not recoloured, not simplified, not redrawn at a different angle.

TAKE HOW ${one ? "IT IS" : "THEY ARE"} DESIGNED — that is the entire reason ${one ? "it is" : "they are"} attached. How much of the frame is left empty. How few colours are used. How quiet and plain the ground stays so that one idea can be loud. How the type is scaled and placed and trusted to carry the frame on its own. How little is happening. Bring all of that across: a banner that shares ${one ? "its" : "their"} restraint, ${one ? "its" : "their"} lightness and ${one ? "its" : "their"} confidence is the target, not a failure.

Then invent content that could belong to no project but this one.`;
  }
...
- Each concept: 50–90 words, concrete enough to execute. Name the register in the first clause, then the composition — what fills the frame, what the subject is doing, where the name sits and how large, the palette. Concrete nouns, no vibes.`,
  ...
  design: {
    // ══ THE THIRD DIRECTOR GIVEN THE PICTURE, AND THE LAST TO GET IT ══
    //
    // This one was deliberately blind, and the reasoning was real: a
    // director who can see the mark tends to echo it, and echoing is
    // most of what makes a banner look assembled rather than designed.
    //
    // What settled it was an A/B nobody planned. The same brief — a
    // liquidity launchpad on Robinhood — went through Default (which
    // sees) and Tek (which did not). Default recognised the Robinhood
    // mark, knew what the chain was, and built concepts around it.
    // Tek, blind, could not know what the logo even depicted, so it
    // invented a world and the renderer bolted the mark on afterwards.
    // "It just does its own thing" was the exact description, and it
    // is what a blind director must do.
    //
    // The old fear is handled the way HIM's was, because HIM taught us
    // the same lesson the hard way: showing a model an image makes it
    // DESCRIBE AND CONTINUE rather than reason, unless the two jobs
    // are separated out loud. So the brief below says plainly what the
    // picture answers — what this project IS — and that the ground
    // remains invented and may still contrast.
    //
    ...
    palette: `- The ${"COUNT"} concepts must be COMPOSITIONALLY unlike each other — different archetypes, not variations of one. Draw from (and go beyond): typography AS the artwork; the mark repeated as pattern or system; a pictorial metaphor filling the frame as a designed ground; an editorial/print layout; a FLAT material study — paper, ink, card, fabric, a printed or painted surface seen face-on; a diagram or geometric system that IS the concept.
- NEVER A MODELLED OBJECT IN A LIT ROOM. This is the one that keeps slipping through, because it sounds like craft: a glass prism on a reflective floor, a metal form under studio lights, a sculpture on a plinth, anything photoreal and three-dimensional with the branding on it. That is a product render, not a designed banner, and a "material study" means a flat surface — never an object built and lit in space. If your concept could be a rendered still from a product launch video, throw it away.
...
- THE PROJECT NAME IS IN EVERY CONCEPT, WHOLE AND READABLE. Say where it sits and how it is set. It may be enormous or small and quiet, but a banner whose job is to identify a project cannot leave the project's name off it, and it is never sliced by the edge of the frame however deliberate that would look. Setting words from the TAGLINE as the artwork is allowed and often strong — but the name still appears alongside them, not instead of them.
...
- A CRYPTO OR INFRASTRUCTURE PROJECT IS NOT A LICENCE FOR ANY OF THAT. It is the exact brief that attracts it, which is why the list exists. The project does something specific — indexes, swaps, lends, stores, routes — and the image comes from THAT, or from what it replaces. If your concept could be described as "a technical-looking surface", you have not had an idea yet.
- A NAME IS NOT A SUBJECT, AND THIS IS THE MISTAKE YOU ARE MOST LIKELY TO MAKE. A launchpad called Honey is not a banner about honey, honeycomb, bees or golden syrup. One called Anchor is not a harbour. One called Forge is not a blacksmith. One whose tagline says "locked" is not padlocks, keys, vaults, chains or safes. Illustrating the word is the first thing that occurs to everyone and it is almost always the weakest concept in the room — it describes the label instead of the company, and it produces a banner that would suit any other project sharing that name. The word may honestly earn a colour or one quiet motif. It never earns the picture.
- THE GROUND IS ONE THING AND IT IS QUIET. Name it in every concept, and name something plain: a flat colour, one honest material, a single photograph, a printed surface, a geometric system. The idea lives in what sits ON that ground and in how much room is left around it. Do not build a scene. Do not fill the frame with invented objects, machinery, architecture or landscape. If a concept needs a busy world to be interesting, it is not finished — the strongest work here is almost empty.
...
- THE TYPE STAYS CLEAN. Brand-grade typography is confident and undecorated — the idea lives in the composition, not in the letterforms. TWO families of decoration are equally fatal and you must propose neither. Distress: flames, cracks, drips, rust, glitching, erosion. Polish: metallic, chrome or gold fills, bevel and emboss, drop shadows and outer glows, extruded or three-dimensional letters, gradients poured down the type, reflections and specular highlights. The second family is the one that slips through, because it looks expensive — it is not, it is a Photoshop layer effect, and it reads as amateur to precisely the clients this style exists for. So never describe the name as gold, metal, chrome, glowing, extruded or 3D — and never as engraved, etched, embossed, debossed, carved, stamped, moulded or cut INTO anything, which is the same failure arriving as a manufacturing process instead of a filter. The name is not part of an object. It is set on the design, in ink, the way type is set. Describe how it is SET: weight, scale, case, tracking, alignment, how much room it is given, what it sits on. If the finished type looks like a layer effect was switched on, the piece has failed however clever the idea behind it was.
- THE CLIENT'S MARK IS ATTACHED, AND IT TELLS YOU WHAT THIS IS — NEVER WHERE IT GOES. Look at it before you write anything. What it depicts, how it is drawn, whether it is a wordmark, a monogram, a symbol or a character, and — if you recognise it — what company or chain it belongs to. That is real knowledge about the project and it belongs in the thinking: a brief that says "a launchpad on Robinhood" plus a mark you actually recognise is a far better starting point than either alone.
- WHATEVER SURROUNDS IT IN THAT IMAGE IS NOT THE BRIEF. A logo arrives on whatever background it was exported on, and continuing that background is the one failure that seeing it makes easy. The ground is still yours to invent and it may still CONTRAST with the mark rather than echo it — a ground chosen to match a logo's colours is most of what makes a banner look assembled rather than designed. Seeing the mark should make the invention sharper, not smaller.
- CHOOSE WHAT THE MARK IS DOING FROM THIS LIST, and name your choice in every concept: placed small against a large field; repeated as a pattern or system; cropped hard by an edge; used as the counter or inside a letterform; sitting as the single element on a plain ground; left out of the frame entirely because the name carries it. This is a list to pick from, not a set of examples to be inspired by.
- THE MARK STAYS A MARK. It is a piece of flat artwork placed on a design, exactly as a designer places a logo on a layout. It is never melted into a scene, reflected in a surface, refracted through glass, embedded in an object, extruded into three dimensions, or reduced to "its colour appearing somewhere". A concept that says the mark is echoed, hinted at or suggested has lost it — the client's identity has to be genuinely, plainly there.
- COLOUR: TAKE HUE, NEVER FINISH. You can see the mark, so you may name its real hues and choose a palette that genuinely works with them — that is the whole gain from being shown it. What you must never take is FINISH: a mark rendered in gold, chrome, gloss or 3D does not make anything else gold, chrome, gloss or 3D, least of all the type. Keep the palette small — two or three colours and one accent — and say which does what.
- Each concept: 50–90 words, concrete enough to execute. Name the idea in one sentence, then the composition — the ground, what fills the frame, where the project name sits and how large, what the mark is doing, and the palette. Concrete nouns, no vibes.`,
  },
...
export async function generateConcepts({ brief, styleBrief, count, constraints = "", perConcept = [], director = "design", logo = null }, retried = false) {
...
Return strict JSON: {"concepts": ["...", "..."]} with exactly ${count} strings. No other text.`;
  ...
  try {
    ...
    // ══ ONE RETRY, AND ONLY FOR AN EMPTY 200 ══
    //
    // Seen in testing: a 200 with finish_reason "stop", eleven
    // completion tokens, and content: null. Nothing to parse. The
    // function returned [] and the run carried on with NO CONCEPT —
    // which is the undirected mark-left/name-right output this whole
    // pass exists to prevent, produced silently, at full price, with
    // nothing on screen or in the log saying the thinking never
    // happened.
    //
    // Deliberately NOT a retry on a bad status. The house rule is that
    // a 429 is never retried because it may be our own billing failing
    // and retrying hides it — the !res.ok branch above still returns
    // immediately. This fires only when the request genuinely
    // succeeded and came back unusable.
    if (out.length === 0 && !retried) {
      console.warn(`[concepts] empty 200 — one retry (${director})`);
      return generateConcepts({ brief, styleBrief, count, constraints, perConcept, director, logo }, true);
    }

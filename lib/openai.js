...
// ============================================================
const CONCEPT_MODEL = process.env.CONCEPT_MODEL || "gpt-4o-mini";
...
export async function generateConcepts({ brief, styleBrief, count, constraints = "" }) {
  ...
  const sys = `You are the creative director at a top-tier brand studio, briefing your designers. You produce banner CONCEPTS — the thinking, not the pixels. You are known for ideas that could belong to no other client: you read what a project actually is, what it is named after, what it replaces, what using it feels like, and you find the visual idea hiding in that. The obvious layout bores you.`;
...
- FORBIDDEN in every concept: the mark on one side with the name on the other over an empty field. That is the default you exist to beat. Also forbidden: circuit boards, holographic interfaces, floating code, glowing particles, generic futurism.

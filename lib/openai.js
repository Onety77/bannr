...
export async function generateConcepts({ brief, styleBrief, count, constraints = "", perConcept = [], director = "design", logo = null }) {
  ...
  //
  // Empty on almost every run. It fills only when a per-option control
  // was given several answers — today that is POV's shot list, where
  // the whole point is seeing three cameras in one run instead of
  // paying three times to see them one at a time.
  //
  // It has to be stated as an assignment rather than a menu. Handed the
  // three shots as a pool, a director writes three concepts drawing on
  ...
  // option 1's camera and a setting demanding its own, which is a
  // contradiction the renderer resolves at random.
  const assigned = perConcept.filter(Boolean).length
    ? `\nSHOT ASSIGNED PER CONCEPT — the client chose these themselves, one per option, and the numbering is fixed. Concept 1 must be built on the first, concept 2 on the second, and so on. This is not a menu to choose from and not a set of suggestions: each concept is being executed with the camera named for it, so a concept describing a different one will be rendered against its own description.

${perConcept.map((p, k) => `Concept ${k + 1}: ${p || "no camera specified — choose one yourself that none of the others has taken."}`).join("\n\n")}

Everything else is still yours. What the subject is doing, where it is, what the light is doing, what keeps it recognisable and where the name sits are all decisions you are making — the camera position is the one thing already decided.\n`
    : "";

...
export async function generateBackground(prompt, { logo, refs = [], reimagine = false, restyle = "" } = {}) {
  if (!aiEnabled()) return null;
  ...
  if (refs.length > 0) {
    finalPrompt +=
      " The remaining attached images are supporting subject/mood guidance only — re-illustrate their spirit as part of the same fused idea, don't collage them in as separate layers.";
  }

  // The logo must stay first — the prompt refers to it positionally
  // as "the first attached image".

...
// ============================================================
import "server-only";
import { PFP_SIZE, PFP_TEXT_MAX, PFP_WANTS_MAX, getPfpStyle } from "./pfpStyles.js";

export { PFP_SIZE, getPfpStyle };
export { PFP_STYLES, PFP_MAX, PFP_COST, PFP_TEXT_MAX, PFP_WANTS_MAX, PFP_IMAGES_MAX, distributeStyles } from "./pfpStyles.js";
...
// ------------------------------------------------------------
// ------------------------------------------------------------
// SEVERAL IMAGES, ONE SUBJECT.
//
// Emitted only when more than one is attached, because the singular
// wording above is clearer when there is only one and a model told to
// "read them together" while holding a single image starts inventing a
// second.
//
// The distinction that matters: these are not mood boards or style
// references. They are the SAME THING photographed more than once,
// and the failure they invite is a collage — two heads, or a subject
// beside a smaller copy of itself.
// ------------------------------------------------------------
function multiBlock(n) {
  if (n < 2) return "";
  return `THERE ARE ${n} IMAGES, AND THEY ARE ALL THE SAME SUBJECT. Different photographs, frames or drawings of one thing — a clearer face in one, the outfit in another, a marking or an object it carries in a third.

Read them together and build ONE understanding of the subject from all of them, then draw that. Where they disagree, trust the sharpest and most complete view. Where one shows a detail the others crop or blur, take the detail from the one that shows it.

They are not separate things to arrange. The result contains ONE subject — not two, not one beside a smaller copy of itself, not a grid, not a collage, not a before and after. If the images genuinely show a character and something belonging to it, the character wears or holds it; they do not sit side by side.

None of them is a style reference. Every one is the subject.`;
}

const SOURCE = `WHAT YOU HAVE BEEN GIVEN. What is attached is whatever the client had to hand, and it is usually not a clean asset. Expect a phone screenshot: a portrait video still or a social post, compressed and soft, with the app's interface sitting on top of the picture. Everything below applies to every image attached.
...
DO NOT DRAW THE CIRCLE. That crop is something the platform does to your square afterwards; it is not something you put in the picture. Deliver a full square image whose artwork runs to all four edges.

Nothing circular is added: no round frame, no ring, no border, no outline, no coloured band around the edge, no circular vignette, no circular mask, no disc the subject sits inside, no transparent or checkered corners, no drop shadow around a round shape, no story-ring, no badge or avatar bubble. The word "profile picture" is not an instruction to draw a profile-picture-shaped object — it is only where the file is going. A square with a circle drawn on it gets cropped into a circle with a circle drawn on it, and looks like a mistake at every size.`;
...
// ------------------------------------------------------------
// ------------------------------------------------------------
// KEEPING THE GROUND IT CAME WITH — THE DEFAULT.
//
// This was opt-in and is now the standing behaviour, which is the
// right way round: the safest thing to do with someone's picture is
// the least to it. Replacing a background is a real change to an
// image they already chose, and a change that size should be
// something they asked for rather than something they have to
// notice and undo.
//
// Only applies to the styles that could invent one. Solid is a flat
// field by definition and Anime redraws the whole frame in another
// medium — in neither is there an original ground left to keep, so
// neither the keep nor the replace block is sent.
//
// It is not a variant of the interface cleanup and must not read as
// one: the app furniture goes either way, and what is being kept is
// the actual place — the room, the street, the sky.
// ------------------------------------------------------------
...
This is not permission to keep the interface. Everything listed above still goes: the overlays, the buttons, the counters, the bars, the captions. What survives is the PLACE, cleaned of everything that was sitting on top of it.

KEPT DOES NOT MEAN UNTOUCHED, and this is the half that decides whether the picture works. It means the setting is not REPLACED — it does not mean it is left alone while the subject gets all the attention. Whatever this style does to the subject is done to the whole frame: the same light, the same colour, the same grade, the same level of finish. If a warm source is rimming the subject, that source exists in the room and the room shows it — spill on the nearest surfaces, the far side falling off, the ambient shifted toward that colour. If the subject has been resolved up out of its compression, so is everything behind it.

ONE PHOTOGRAPH, NOT A SUBJECT PASTED ONTO ITS OWN BACKGROUND. The failure here is a sharp, lit, treated subject sitting in front of a flat dull murky rectangle that was clearly left over from the source — and it is instantly obvious, because nothing in the world looks like that. The subject and its ground must share one light, one palette and one depth of field, and read as a single image that was always this way.

The ground still stays quieter than the subject. Push it back with light, focus and contrast rather than by neglecting it: softened, dimmer, less saturated, less resolved — deliberately subordinate, never merely unfinished. Where the treatment above asked for a plain, dark or empty ground, read that as an instruction about ATTENTION rather than about content: the real place stays, graded down until it is as quiet as a plain one would have been.`;

// The other side of the toggle. Worth saying explicitly rather than
// just omitting KEEP_BG: someone who asked for a new background
// asked for a real one, and silence here leaves the model free to
// keep most of the old scene and call it changed.
const NEW_BG = `REPLACE THE BACKGROUND. Whatever the subject was originally photographed or drawn against does not survive — not softened, not blurred, not partially kept. Build a new ground for it, chosen by the treatment above, and let none of the original setting show through.`;

// ------------------------------------------------------------
// WHAT THEY ASKED FOR, IN THEIR OWN WORDS.
//
// Default only. The other three styles ARE the instruction, and
// inviting a second brief on top of "make it anime" produces two
// directions arguing inside one prompt.
//
// Placed near the end for the reason the banner's own direction
// field is: models weight what they read last, and this is the one
// part of the brief written about this specific picture. It still
// sits above the framing and fidelity rules, which are technical.
// ------------------------------------------------------------
function wantsBlock(wants) {
  if (!wants) return "";
  return `WHAT THE CLIENT ASKED FOR — in their own words:

${wants}

This is a direct instruction about this specific picture and it is the most important creative input here. Follow it, and where it disagrees with the treatment above, it wins.

Interpret it intelligently rather than literally — they are describing an outcome, not writing a specification. It does not override the two rules below: the subject stays recognisably theirs, and no interface, lettering or circular frame appears.`;
}

export function buildPfpPrompt(styleId, { text = "", color = "", newBg = false, wants = "", images = 1 } = {}) {
  const style = getPfpStyle(styleId);
  const clean = String(text || "").trim().slice(0, PFP_TEXT_MAX);
  const want = String(wants || "").trim().slice(0, PFP_WANTS_MAX);

  let treatment = STYLES[style.id] || STYLES.default;
  // Keeping it is the default; only a style that COULD invent a ground
  // gets either block.
  if (style.keepBg) treatment += `\n\n${newBg ? NEW_BG : KEEP_BG}`;
  if (style.id === "solid") {
    treatment += color
  ...
  return [
    ...
    multiBlock(Number(images) || 1),
    SOURCE,
    FIDELITY,
    treatment,
    // Only Default carries one, and only when they wrote something.
    style.wants ? wantsBlock(want) : "",
    FRAME,
    ...
    // than merely worse.
    textBlock(clean),
    `FINALLY, AND OVER EVERYTHING ABOVE: the subject must be recognisably the one attached, no part of any app interface, screenshot furniture or unrequested lettering may appear anywhere in the result, and the image is a full square with no circular frame, ring or mask drawn on it.`,
    // multiBlock and wantsBlock return "" most of the time; joining
    // those in leaves blank paragraphs the model reads as separators
    // between sections that are not there.
  ].filter(Boolean).join("\n\n");
}

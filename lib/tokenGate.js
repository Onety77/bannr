...
//
// EVERYTHING TOKEN-RELATED IS OFF BY DEFAULT. No mint, not enabled,
// nothing announced. Until an admin fills this in there are no tiers
// and no discounts.
//
// THE FREE TIER IS THE EXCEPTION and it is deliberate. One run a day
// for anyone signed in does not depend on a token existing, so it sits
// in `free` and is read straight past `enabled`. See lib/tiers.js.
// ============================================================
import "server-only";
import { getAdminDb } from "@/lib/firebaseAdmin";
import { cleanTiers, cleanFree, DEFAULT_TIERS, DEFAULT_FREE, tierFor } from "@/lib/tiers";
...
// case, not the average. At ~$0.31 for a three-option run — which is
// what a free run makes — 500 free runs a day is roughly $154/day.
// COST_PER_RUN in app/api/admin/token/route.js is the one place that
// number is written down; this note quotes it and nothing computes
// from it here.
export const DEFAULT_GATE = {
  enabled: false,        // master switch for the HOLDER tiers
  ...
  // Three rungs, ascending, thresholds in whole tokens. See
  // lib/tiers.js for why they are not in dollars.
  tiers: DEFAULT_TIERS,
  // What a signed-in non-holder gets. Independent of `enabled`.
  free: DEFAULT_FREE,
  ...
  // ══ THE COMMITMENT ══
  //
  // The share of PRODUCT revenue — money from customers buying credit
  // packs, not trading fees — promised to buying the token back.
  //
  // It lives in config rather than in code because it is a public
  // promise and the number will be argued about; changing it must not
  // need a deploy, and must be visible in the same panel that shows
  // whether it is being kept.
  //
  // 0 means no commitment is published at all, which is the honest
  // default: a percentage stated before anyone intends to honour it is
  // worse than saying nothing.
  buybackPct: 0,
  // ══ WHEN TO ACT, IN REVENUE RATHER THAN IN TIME ══
  //
  // "Every 1 SOL earned" rather than "every Friday". A calendar fires
  // on a quiet week with nothing to buy and stays silent through a
  // busy Tuesday, so the cadence follows the takings instead.
  //
  // Expressed in SOL OF REVENUE because that is the number in your
  // head; the obligation it implies is that times buybackPct, and
  // commitment() does the multiplication. Setting it here in owed-SOL
  // instead would mean re-deriving it by hand every time the
  // percentage moved.
  buybackEverySol: 1,
...
const NUMERIC = {
  ...
  buybackPct:      { min: 0, max: 100 },
  // Floored at 0.01 rather than 0. A threshold of zero means "always
  // due", which is not a cadence — it is a permanently lit warning
  // light, and a warning light that is always on gets ignored.
  buybackEverySol: { min: 0.01, max: 10000 },
};

// The bar to reach the ladder at all — the first rung's threshold.
// Read it through here rather than reaching for tiers[0] everywhere,
// because "the entry threshold" is a concept several callers need and
// "index zero" is an implementation detail that would be wrong the day
// a fourth tier is added at the bottom.
export function entryBar(gate) {
  return (gate?.tiers || []).reduce(
    (min, t) => (t.minTokens > 0 && (min === 0 || t.minTokens < min) ? t.minTokens : min),
    0
  );
}
...
export function cleanGate(raw = {}) {
  ...
  out.tiers = cleanTiers(raw.tiers);
  out.free = cleanFree(raw.free);
  ...
  // an honest off.
  //
  // "Grants something" is now two different things — free runs OR a
  // discount — and a tier offering only a discount is a perfectly good
  // tier. Requiring dailyRuns > 0 (which is what this used to do)
  // would refuse to arm a ladder built entirely on discounts, which is
  // the shape we may well end up wanting.
  const bar = entryBar(out);
  const grantsSomething = out.tiers.some((t) => t.minTokens > 0 && (t.dailyRuns > 0 || t.discount > 0));
  out.enabled = Boolean(raw.enabled) && Boolean(out.mint) && bar > 0 && grantsSomething;
  return out;
}
...
//
// THE TIER TABLE IS PUBLIC, in full, including thresholds. It has to
// be — "hold this much for this" is the offer, and an offer you cannot
// read before signing in is not an offer. What stays back is only the
// machinery that limits it.
//
// `free` is published whether or not the gate is live, and the tier
// list collapses to empty when it is not. Off, this route says exactly
// one true thing: everyone gets one run a day and there is nothing
// else on the table yet.
export function publicGate(g) {
  const free = cleanFree(g.free);
  return {
    live: g.enabled,
    ...
    name: g.name,
    free,
    tiers: g.enabled ? g.tiers : [],
    // The entry bar and the top allowance, pulled out because most of
    // the copy on the site wants one or the other and should not have
    // to reduce an array to get it.
    minTokens: g.enabled ? entryBar(g) : 0,
    dailyRuns: g.enabled ? Math.max(0, ...g.tiers.map((t) => t.dailyRuns)) : 0,
    maturityHours: g.enabled ? g.maturityHours : 0,
    // NOT gated on `enabled`. The buyback commitment is about revenue,
    // not about who generates for free, and the two are switched on at
    // different moments — the promise can be live before the ladder is.
...
//
//   { qualified, tier, balance, min, wallet, reason, maturesAt }
//
// `tier` is the row from lib/tiers.js, or null. `qualified` is kept
// and means "reached at least the first rung" — it is what decides
// whether anything is granted at all, while `tier` decides how much.
//
...
export async function evaluate(accountId, wallets, gate) {
  ...
  const bar = entryBar(g);
  if (!g.enabled) return { qualified: false, tier: null, reason: "off", balance: 0, min: 0 };

  const list = (wallets || []).filter(isAddress);
  if (!list.length) return { qualified: false, tier: null, reason: "no-wallet", balance: 0, min: bar };
  ...
  if (!known.length) {
    ...
    return { qualified: false, tier: null, reason: "unknown", balance: 0, min: bar };
  }
  ...
  if (balance < bar) {
    const db = getAdminDb();
    if (db) await touchHolder(db, g.mint, best, accountId, bestAmt < 0 ? 0 : bestAmt, bar).catch(() => {});
    // The balance rides along even on a refusal, because it is what
    // the credits page needs to say how far off the first rung is.
    return { qualified: false, tier: null, reason: "short", balance, min: bar, wallet: best };
  }

  if (g.maturityHours > 0) {
    const db = getAdminDb();
    if (db) {
      const since = await touchHolder(db, g.mint, best, accountId, bestAmt, bar).catch(() => 0);
      const readyAt = (since || Date.now()) + g.maturityHours * 3600_000;
      if (Date.now() < readyAt) {
        return { qualified: false, tier: null, reason: "maturing", balance, min: bar, wallet: best, maturesAt: readyAt };
      }
    }
  } else {
    const db = getAdminDb();
    if (db) await touchHolder(db, g.mint, best, accountId, bestAmt, bar).catch(() => {});
  }

  // MATURITY IS MEASURED AGAINST THE ENTRY BAR ONLY, not against the
  // tier reached. Someone who matured at t1 and then bought up to t3
  // gets t3 immediately. Restarting the clock on every top-up would
  // punish exactly the behaviour the ladder exists to encourage, and
  // the thing maturity defends against — one bag walked between fresh
  // wallets — is already stopped at the first rung.
  return { qualified: true, tier: tierFor(balance, g.tiers), reason: "ok", balance, min: bar, wallet: best };
}

...
//      a session token.
//   3. Every later request — signMessage, signAndSendTransaction — is
...
//   { type: 'signature', provider, signature, label }   ← a tx signature
...
function clearState(provider) {
  saveState(provider, { secretKey: null, sharedSecret: null, session: null, publicKey: null, pending: null });
}
...
export function restoreSession(provider) {
  ...
  if (publicKey && session && sharedSecret) return { publicKey, session };
  return null;
}
...
/** Sign and send a web3.js Transaction via app-hop. */
export function signAndSendTransaction(provider, transaction, label = "") {
  const def = PROVIDERS[provider];
  if (!def) throw new Error(`Unknown deeplink provider "${provider}"`);
  const session = getState(provider, "session");
  if (!session) throw new Error("Wallet session expired. Connect again.");
  ...
  navigate(`${def.base}/signAndSendTransaction?${params}`);
}
...
export function handleRedirect() {
  ...
  try {
    ...
    if (action === "sign") {
      ...
      const label = getState(provider, "pending");
      saveState(provider, { pending: null });
      const detail = { type: "signature", provider, signature: data.signature, label };
      emit(detail);
      return detail;
    }
  } catch (err) {

...
//      a session token.
//   3. Every later request — signMessage is the only one left — is
...
// ══ THERE IS NO TRANSACTION METHOD HERE ANY MORE ══
//
// There were two, in turn, and both are gone. signAndSendTransaction
// was retired by Phantom in July 2025 and started answering "method
// not supported" after the user had already approved. signTransaction
// replaced it and worked, but it left US choosing the blockhash — and
// a blockhash lives about a minute, while the round trip through a
// wallet app is a person reading a warning about their own money. It
// expired more often than it landed.
//
// Paying is now a Solana Pay transfer request: the wallet is handed
// the intent and builds, signs and sends the transaction itself, with
// a blockhash it fetches at the moment of approval. See
// lib/solanaPay.js. Nothing about a payment comes back through this
// file, so nothing about a payment belongs in it.
//
// What remains is signMessage, because proving you own a wallet needs
// a signature over a server-issued nonce and a transfer request cannot
// give one.

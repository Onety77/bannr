...
import { readRunToken } from "@/lib/auth";
...
// ══ A CREDIT BACK FOR PUTTING A BANNER ON THE FEED ══
//
// The feed is what makes this look like a product rather than a tool,
// and it only fills if posting is worth doing. One credit back is
// roughly a third of a run — enough to notice, not enough to farm.
//
// TWO LIMITS, AND EACH ONE CLOSES A DIFFERENT HOLE.
//
// Once per RUN, so posting the second option from the same run pays
// nothing. Without it a single run of four banners would pay four
// credits — more than the run cost — and the cheapest way to get
// credits would be to stop generating and start posting.
//
// Three a day, so the most anyone can earn back is three credits,
// which is one free run's worth. Posting is still allowed after that
// and still shows on the feed; it just stops paying.
//
// The cost is real and worth stating plainly: three credits a day is
// about 31 cents of generation per person who does it every day, on
// top of whatever their tier already grants.
export const POST_REWARD_CREDITS = 1;
export const REWARDED_POSTS_PER_DAY = 3;
...
// The source logo, shown over the artwork on the card.
//
// SAME SUSPICION AS THE BANNER ITSELF: it arrives from a browser, so
// it must be an inline image and it must be small. A URL here would
// let a post point the card at any host on the internet and turn
// every reader into a request to it; a large one would push the
// document toward Firestore's 1MB limit and take the post with it.
const MAX_LOGO = 60_000; // base64 chars; a 256px jpeg is ~15-25k
function logoOf(v) {
  const s = String(v || "");
  if (!s.startsWith("data:image/") || s.length > MAX_LOGO) return null;
  return s;
}
...
export async function publish(accountId, body = {}) {
  ...
  // Verified before anything is written, and never trusted as given:
  // a run id the browser invented is a credit printer. See
  // issueRunToken in lib/auth.js.
  const runId = readRunToken(accountId, body.runToken);
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    // ══ IS THIS POST WORTH A CREDIT ══
    //
    // Decided in the SAME transaction that writes the post and the
    // count, so two requests racing cannot both find the last paid
    // slot free, and a post can never be written without its payment
    // being settled one way or the other.
    //
    // `paidRuns` holds the run ids already paid today and resets with
    // the daily count, which is the whole reason a run token is only
    // valid on the day it was issued: the list can never grow past
    // REWARDED_POSTS_PER_DAY entries.
    const sameDay = d.rewardDate === today;
    const paidRuns = sameDay && Array.isArray(d.paidRuns) ? d.paidRuns : [];
    const earn =
      Boolean(runId) &&
      !paidRuns.includes(runId) &&
      paidRuns.length < REWARDED_POSTS_PER_DAY;

    tx.set(postRef, {
      accountId,
      src,
      // The picture the banner grew from, square and small. The card
      // lays it over the artwork — nothing is baked into the image,
      // which stays exactly what was made.
      logo: logoOf(body.logo),
      // Width ÷ height, for the handful of posts made during the week
      // the logo WAS painted into a taller image. Nothing produces
      // one now; it stays so those posts still reserve their real
      // height instead of being cropped to 3:1. Absent means 3:1.
    ...
    tx.update(userRef, {
      ...
      ...(earn
        ? {
            rewardDate: today,
            paidRuns: [...paidRuns, runId],
            credits: (d.credits || 0) + POST_REWARD_CREDITS,
          }
        : {}),
    });
    return {
      ok: true,
      id: postRef.id,
      earned: earn ? POST_REWARD_CREDITS : 0,
      // What is left today, so the client can say "two more" rather
      // than only reporting the one just paid.
      rewardsLeft: Math.max(0, REWARDED_POSTS_PER_DAY - (paidRuns.length + (earn ? 1 : 0))),
    };
  });

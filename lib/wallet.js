...
// Build the treasury payment, without sending it.
//
// SEPARATE FROM SENDING because the deeplink path needs it to be. A
// transaction needs a blockhash, fetching one is an await, and on a
// phone the navigation to the wallet must happen inside the tap or
// iOS opens phantom.app as a web page instead of the app. So this
// runs on page load and the result waits for the tap.
//
// `accountId` is stamped in as a memo. That is what makes wallet
// linking unnecessary for payment: the transaction names the account
// it belongs to, so any wallet can pay for any account and nobody
// else can claim it. It costs the payer nothing — same single
// approval, one extra instruction inside it.
export async function buildTreasuryTx(sol, accountId, fromAddress) {
  const treasury = process.env.NEXT_PUBLIC_TREASURY_WALLET;
  if (!treasury) return { error: "Treasury wallet not configured yet (demo mode)." };
  if (!fromAddress) return { error: "Connect a wallet first." };
  try {
    ...
    return { error: e.message || "Couldn't build that payment." };
  }
}
...
export function useWallet() {
  ...
  useEffect(() => {
    ...
    if (p.publicKey) {
      ...
      // ══ publicKey IS NULL ON EVERY PAGE LOAD ══
      //
      // Even with the extension installed, unlocked, and the site
      // already approved. A wallet does not hand out an address just
      // because a page exists — it waits to be asked. So reading
      // publicKey directly meant the answer was "no wallet here" on
      // every first render, and the linked-wallet dot was grey on a
      // desktop with Phantom open two inches away.
      //
      // onlyIfTrusted is the ask that costs nothing: it resolves
      // silently for a site the user has already approved and
      // rejects silently otherwise. NO POPUP either way, which is
      // what makes it safe to do unprompted on page load.
      try {
        p.connect?.({ onlyIfTrusted: true })
          ...
          .catch(() => {}); // not trusted yet — the normal case
      } catch {}
    }

    // And keep it honest afterwards. Switching accounts inside the
    // wallet, or disconnecting from its own UI, happens outside this
    // page entirely; without these the dot would keep describing a
    // state that ended minutes ago.
    const onConnect = () => setAddress(provider()?.publicKey?.toString() || null);
    const onDisconnect = () => setAddress(null);
    const onAccount = (key) => setAddress(key ? key.toString() : null);
  ...
  // Send SOL to the treasury (buy credits), through an injected
  // provider. The deeplink path builds the same transaction with
  // buildTreasuryTx above and hands it to the wallet app instead.
  const payTreasury = useCallback(async (sol, accountId) => {
    const p = provider();
    if (!p?.publicKey) return { error: "Connect a wallet first." };
    const built = await buildTreasuryTx(sol, accountId, p.publicKey.toString());
    if (built.error) return { error: built.error };
    try {
      // The wallet broadcasts through its OWN RPC connection, so we
      // never need one client-side beyond the blockhash.
      const { signature } = await p.signAndSendTransaction(built.transaction);
      return { signature };
    } catch (e) {

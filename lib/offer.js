...
// for everyone and two on the first rung, "hold 1K $BANNR for 2 free
// banners a day" describes one more than the reader already has.
//
// So the sentence is built from the DELTA. It used to name unlocked
// features here too — every style, your own direction — and those are
// gone: no feature is token-gated, so a rung is extra runs, a cheaper
// pack, and status. See lib/tiers.js.
export function offerLine(t) {
  ...
  const list = gains.length === 1 ? gains[0] : `${gains[0]} and ${gains[1]}`;

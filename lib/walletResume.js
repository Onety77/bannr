...
export function resumeOnce() {
  ...
  (async () => {
    ...
    // A broadcast transaction. Confirming it is the credits page's
    // job, because it owns the packs and the claim.
    if (result.type === "signature") {
      setWalletState({
        pendingPay: null,
        busy: false,
        paid: { signature: result.signature, payload: pending?.payload || null },
      });
      return;
    }

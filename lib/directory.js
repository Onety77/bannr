...
export async function directory() {
  ...
  try {
    ...
    snap = await db.collection("posts").orderBy("ts", "desc").limit(SCAN).get();
  } catch {
    return { tokens: [], scanned: 0 };
  }

  // One row per TOKEN, not per banner. A project that posted six
  // options is one project; the newest banner represents it.
  const byCa = new Map();
  for (const doc of snap.docs) {
    const p = doc.data();
    if (!p?.ca || p.hidden) continue;
    if (byCa.has(p.ca)) continue;
    byCa.set(p.ca, { ca: p.ca, src: p.src, ticker: p.ticker || "", postId: doc.id });
  }

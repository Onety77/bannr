...
// Priced by option count, and the free-run ceiling. Both live in
// lib/packs.js because the create page must show them and this file
// is server-only; re-exported here so server code has one import.
export { runCost, FREE_RUN_MAX_OPTIONS } from "@/lib/packs";
...
// Exported because two other modules had quietly grown their own copy
// of this line. Three definitions of "which day is it" is two chances
// for an allowance to roll over at a different moment than the thing
// reading it.
...
export async function consumeGeneration(accountId, { allowance = 0, globalCap = 0, cost = GENERATION_COST } = {}) {
  const today = todayKey();
  const db = getAdminDb();
  ...
  if (!db) {
    ...
    if ((u.credits || 0) < cost) return { ok: false };
    u.credits -= cost; mem.set(accountId, u);
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (used < allowance) {
      ...
      if (credits < cost) return { ok: false, capped: true };
      tx.update(uref, { ...base, gateUsed: used, credits: credits - cost });
      return { ok: true, paidWith: "credits", credits: credits - cost, holderLeft: allowance - used, capped: true };
    }

    if (credits < cost) return { ok: false, holderLeft: 0 };
    tx.update(uref, { ...base, gateUsed: used, credits: credits - cost });
    return { ok: true, paidWith: "credits", credits: credits - cost, holderLeft: Math.max(0, allowance - used) };
  });
}

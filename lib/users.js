...
// The account this identity already opens, or null. No creation.
//
// Wallet sign-in uses this rather than getOrCreateByIdentity, because
// a wallet may no longer OPEN an account — only join one. See
// /api/auth/verify.
export async function getByIdentity(type, id) {
  const existing = await resolveIdentity(type, id);
  if (!existing) return null;
  return (await getUser(existing)) || null;
}

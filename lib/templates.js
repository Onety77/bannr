...
// "Normal" — no style guidance at all; the model invents the
...
// one of them. Renamed from "Auto" for users; the stored id stays
// "auto" so existing history and shared /create?style= links keep
// resolving.
export const AUTO_ID = "auto";
export const AUTO_NAME = "Normal";
...
export const TEMPLATES = [
  {
    id: "tech",
    name: "Tech",
    tagline: "Brand-grade. Type-led.",
    // The doctrine tells every style to infer the WORLD around the
    // subject and build a banner expressing it. That is right for
    // Meme, Anime and Glow and precisely wrong here — this style is a
    // designed surface, not a depicted place, and without the
    // override the model keeps staging the logo somewhere.
    graphic: true,
    // Banners in this register almost always carry a short line under
    // the name. When the client hasn't written one, it is better to
    // compose it from their description than to leave the layout
    // short of the element it is built around.
    wantsTagline: true,
    accent: "#2451FF",
    thumb: "tech.jpg",
    mood: `Design a brand banner with the confidence of the work premium technology, finance and culture companies put out for themselves — a launch header, a keynote title card, a campaign image. Restraint, craft and taste, with nothing to prove.

START BY THINKING, NOT BY LAYING OUT. Read the briefing and work out what this specific project actually is. Then find the one visual idea that belongs to it and to nothing else — drawn from what it does, what it is named after, the thing it replaces, the feeling of using it, or the world it comes out of. The composition is the CONSEQUENCE of that idea. Two different projects must never arrive at the same banner. You are smart enough to have a real idea here; have one.

THERE IS NO HOUSE LAYOUT. The arrangement is a decision you make freshly for this project, never a template you fill in. In particular: the mark on one side with the name and a line on the other is the most obvious answer available and almost never the most interesting one. Reject it unless this project genuinely demands it. A banner that could be swapped onto another project by changing the words is a failed banner.

All of these are legitimate, and they are a sample of what is open to you, not a list to choose from:
— an enormous quiet field with one small element placed with total precision
— type broken apart and set far across the frame, using the full 3:1 width as the idea itself
— the name hard against one edge, its weight answered by emptiness
— words set at an angle, on a curve, or running vertically
— a strict grid or modular system that the mark and the type sit inside
— type in perspective, receding into depth or wrapping a form
— the mark cropped enormous, only part of it inside the frame
— a single photographic ground — a landscape, a desert, an ocean, an architectural surface, a material in close-up — art-directed and graded, with the type placed on it the way a designer would place it
— the mark repeated as pattern, or reduced to pure geometry
Pick what the idea wants, and invent arrangements that are not on this list.

THE UPLOADED IMAGE IS A BRAND MARK. Place it, scale it, crop it, silhouette it, repeat it, or abstract it to its essential geometry with the same discipline you give the type. It is never a character posed in a scene and never a 3D object sitting on a plinth. Where a scene does appear it is a GROUND the composition sits on, not a place the mark is standing in.

TYPOGRAPHY IS THE CRAFT. Real decisions — considered scale, precise letter-spacing, deliberate weight, intentional alignment. The name is usually the anchor, but where it sits, how large it runs and how it relates to everything else is yours to decide. Supporting lines are subordinate to the name and must still be comfortably readable.

Avoid circuit boards, floating code, holographic interfaces, hexagon grids, binary text, glowing particles, dashboard mockups and decorative sci-fi entirely. Nothing here should look rendered by default. It should look designed.`,
...
// ------------------------------------------------------------
// THE CREATIVE DOCTRINE — sent to every real AI generation. Every
// category adds one short mood note on top of this; Auto adds none
// at all and leaves the whole direction to the model. Nothing here
// dictates colors, coordinates, or a checklist of required text —
...
export function buildPrompt(template, brief, variantSeasoning = "", { auto = false, settings = {} } = {}) {
...
${
  writeTagline
    ? `
This style is built around a short supporting line beneath the name, and the client did not write one. Compose it yourself from the briefing below — at most six words, plain and confident, saying what the project actually is or does. Set it clearly subordinate to the name, but comfortably readable. No hype, no exclamation, no invented statistics, no claims the briefing does not support. This is the single exception to the rule against adding words; everything else below still applies.
`
    : ""
}
${
...
LEGIBILITY FLOOR. Every word you do set must be readable at a glance. These banners are seen small — a strip a few centimetres wide on a phone, scrolled past in a second. Supporting lines sit below the name in HIERARCHY, which is a matter of scale, weight and placement — not a licence to set them so fine, so faint, so tightly tracked or so low in contrast that the viewer has to work at them. A subordinate line should still be read without effort. If a line is not worth setting at a readable size, leave it out altogether.`
  ...
  parts.push(
    about
      ? `BRIEFING — context only. None of the following is ever rendered:

${about}

This is the client explaining what the project is and why it exists, so that you understand the world you are designing for. It is a description written FOR you, not copy to place ON the banner. Do not letter these words, or paraphrases of them, into the image. Let it shape the concept, mood, setting, palette, imagery and story — then let the words themselves disappear.`
...
No description was given. Infer the project's world from its name and the uploaded image alone, and commit to your reading of it.`
  ...
  // The doctrine opens by telling the model to infer the WORLD around
  // the subject and build a banner expressing it. Read literally by a
  // design-led style that produces the one failure this style cannot
  // afford: the logo staged somewhere, standing in a room or on a
  // plinth, as though it were a character.
  //
  // Note what this override does NOT say. An earlier version banned
  // scenes outright — no landscape, no horizon, no photography. That
  // went too far: a graded desert or ocean under well-placed type is
  // a completely legitimate answer, and forbidding it removed one of
  // the strongest arrangements available. The real distinction is not
  // scene vs no scene, it is GROUND vs HABITAT.
  if (template?.graphic) {
    parts.push(
      `COMPOSITION, NOT HABITAT — this overrides the instruction to build the world around the subject.

Do not read the uploaded image as a character and build a place for it to live in. It is a brand mark. Place it, scale it, crop it, silhouette it, repeat it or reduce it to its essential geometry, and compose type, shape, colour and space around it the way a design team lays out a launch banner.

An environmental or photographic ground is allowed where the idea genuinely earns it — a landscape, an ocean, a material, an architectural surface — but it is a GROUND the composition sits on, art-directed and graded into the palette. It is never a habitat the mark is standing in, posed on, or staged inside.`
    );
  }

...
export const TEMPLATES = [
  {
    ...
    accent: "#2451FF",
    thumb: "tech.jpg",
    mood: `Design a brand banner in the register premium technology, finance and media companies use for their own marketing: a launch page header, a keynote title card, a campaign banner. Restraint, craft and confidence — the look of a company with taste and nothing to prove.

TYPE LEADS. The project name is the primary element, set with real typographic care — considered scale, precise letter-spacing, deliberate weight, clean alignment. Everything else is arranged around it. A supporting line, where there is one, sits in clear hierarchy below: smaller, quieter, often letter-spaced or in small caps.

TREAT THE UPLOADED IMAGE AS A BRAND MARK, NOT A CHARACTER. It is a logo to be placed, scaled, silhouetted, cropped, tiled, repeated as pattern or abstracted into geometry with the same discipline as the type. It is never a figure standing somewhere, never a posed 3D object, and it is never given an environment to inhabit.

Build the surface from graphic material rather than from a place: flat colour fields, controlled gradients, duotone treatments, hard-edged geometry, arcs, grids, stripes, repetition, pattern, and generous emptiness. One or two colours doing all the work beats many. A single small mark on a vast quiet field is a legitimate and often superior answer.

THERE IS NO SCENE. No landscape, no horizon, no sky, no space, no room, no architecture, no desk, no device, no hardware, no product mockup, no character in a setting, no cinematic depth of field. If photographic material appears at all it must be heavily treated — duotoned, posterised, graded into the palette — and it must sit beneath the typography rather than compete with it.

Avoid circuit boards, floating code, holographic interfaces, hexagon grids, binary text, glowing particles and decorative sci-fi entirely.`,
...
No description was given. Infer the project's world from its name and the uploaded image alone, and commit to your reading of it.`
  ...
  // the subject and build a banner expressing it. That is the right
  // instinct for most styles and exactly wrong for a design-led one,
  // where the answer is a composed surface rather than a depicted
  // place. Without this override the model keeps staging the logo
  // somewhere — a room, a landscape, a plinth — because it was asked
  // for a world and is obliging.
  if (template?.graphic) {
    parts.push(
      `SURFACE, NOT WORLD — this overrides the instruction to build the world around the subject.

Do not infer a place for this project and depict it. There is no environment, no setting and no scene here. What you are composing is a designed surface: type, shape, colour and space arranged on a flat plane, the way a brand team lays out a launch banner.

The uploaded image is a mark to be composed WITH, not a character to be staged. Place it, scale it, crop it, silhouette it, repeat it, or reduce it to its essential geometry — but never stand it somewhere or build a world for it to live in.`
    );
  }

...
Cinematic is a craft, not a filter, and the difference decides whether this looks expensive or cheap. It is the lens, the light and the moment. It is NOT teal-and-orange grading, lens flares, letterbox bars, grain laid over the top, or "epic" as a mood — those are what gets added when the framing did not do the work, and they read instantly as an effect switched on. If it would still hold as a film still under flat grey light, the framing is right.`,
  ...
  },
  {
    id: "glow",
    name: "Glow",
    tagline: "Light on the edge, not over it.",
    accent: "#7C5CFF",
    thumb: "glow.jpg",
    ...
    mood: `The subject is edged in light. A thin bright rim traces its silhouette — the contour catching a source set behind it and off to one side — and that rim IS this style. Everything else exists to let it read.

Be exact about where the light lands, because the word glow is usually taken to mean something else. This is not haze in the air. Not bloom. Not a halo or aura floating around the subject as a separate shape. Not a bright sky or a soft sun behind it. Not a neon outline drawn on afterwards. It is light striking the subject's own edge — along the fur, the shoulder, the jaw, the lip of the object — so the form lifts cleanly off what sits behind it.

A real rim is uneven, and that is the difference between light and a sticker stroke. It is brightest where the surface turns away from the viewer toward the source, thins as the form rolls back toward the camera, and breaks off completely where the silhouette faces away. Never a band of even thickness running the whole way round.

RESTRAINT IS THE STYLE, not a caveat on it. The rim occupies a fraction of the frame. The ground stays dark or muted so the edge has something to be seen against, and no second light competes with it. If the picture looks lit up, it has failed. The subject should be noticed first and the light second — someone should feel the separation before they work out how it was done.

...
export const TEMPLATES = [
  {
    ...
    // subject and build a banner expressing it. Read literally by a
    // design-led style, that produces the one failure this style
    // cannot afford: the logo treated as a character and given a
    // place to stand. The override keeps it a MARK. It does not
    // forbid environments — this style is free to use them.
    graphic: true,
...
${
  writeTagline
    ? `
The client did not write a tagline, and on this style you are permitted to author one. If a short supporting line would strengthen the composition, compose it from the briefing below — at most six words, plain and confident, saying what the project actually is or does. Set it subordinate to the name but comfortably readable, and place it wherever the composition wants it rather than automatically beneath the name. If the design is stronger with no supporting line at all, leave it out. No hype, no exclamation, no invented statistics, no claims the briefing does not support. This is the single exception to the rule against adding words; everything else below still applies.
`
    : ""
}
${
...
No description was given. Infer the project's world from its name and the uploaded image alone, and commit to your reading of it.`
  ...
  // The doctrine opens by telling the model to infer the WORLD
  // around the subject and build a banner expressing it. Read
  // literally by a design-led style, that produces the one failure it
  // cannot afford: the logo treated as a character and handed a place
  // to stand — a room, a plinth, a landscape it is posed in.
  //
  // Note how narrow this override has become. An earlier version
  // banned scenes outright — no landscape, no horizon, no photography
  // — and that was over-correction: it removed cinematic imagery,
  // which is one of the strongest answers available here. The line is
  // not scene vs no scene. It is GROUND vs HABITAT, and that is the
  // only thing this block exists to say.
  if (template?.graphic) {
    parts.push(

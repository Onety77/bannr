...
export const TEMPLATES = [
  {
    ...
    soloBrief: true,
    // ══ THE DIRECTOR DOES THE THINKING; THE RENDERER DRAWS ══
    //
    // gpt-4o reads a brief far better than an image model does, and it
    // was already writing the concept. Under this flag it writes the
    // WHOLE brief: the renderer is sent that concept, the name, the
    // mark and the rules above, and none of the reasoning that
    // produced the idea.
    //
    // Measured before it was built. On the same three projects the
    // lean prompt beat the full one every time, and the control fell
    // into the generic-futurism trap its own ban was written to
    // prevent — a rule that is present and still broken is the
    // signature of a prompt with too much in it to weigh.
    //
    // 13,019 characters to 8,264, and the concept got LONGER.
    leanBrief: true,
    ...
    forbid: [
      ...
      // The 3D-render ban that sat here is GONE, and its removal is
      // the point. It was written to stop brass doors and it stopped
      // everything: a photographic landscape carrying real tickers,
      // a lit environment with an idea in it, any pictorial ambition
      // at all. Tek's best work has always been rich — the ban that
      // actually fixed the Honey run is the one below, about the name.
...
Approach every project as a unique design problem rather than a template to fill. Before placing a logo, image, or text, identify the strongest visual opportunity hidden within the project's identity, product, story, or purpose, and let that idea drive the entire composition. The goal is not simply to make a clean technology banner, but to create something that feels inseparable from that specific project — as if no other brand could own it. Sometimes the typography becomes the artwork, sometimes the environment tells the story, sometimes the product itself defines the layout, and sometimes the most powerful decision is restraint. Do not default to placing a logo beside a title and tagline. Instead, explore composition freely: text may be centered, oversized, tiny, rotated, integrated into the environment, layered behind subjects, aligned to architectural lines, or placed in unexpected corners. A banner should feel discovered rather than assembled.

THE PROJECT NAME ALWAYS APPEARS, WHOLE AND LEGIBLE. It may be enormous or it may be small and quiet in a corner, but it is on the banner and it can be read at a glance. This is what the banner is FOR: somebody scrolling a token list has to know whose it is. A supporting line may be dropped whenever it does not earn its place — the name may not. And every letter of it sits fully inside the frame: nothing runs off an edge, nothing is sliced by the crop, however deliberate that might look.
...
ONE IDEA, WHOLLY COMMITTED TO. The frame holds a single thought and everything in it serves that thought — not a flat colour with a logo dropped on it, and not a pile of unrelated objects either. A landscape, a photograph, an object, a diagram, a pattern, a typographic system or a plain field are ALL legitimate grounds; what decides between them is the idea, never a preference for emptiness. Ambition and atmosphere are welcome. Restraint means nothing arbitrary in the frame, and it does NOT mean an empty frame: a banner with nothing in it has no idea in it either, and a bare colour field with a mark and a name is the assembled default this style exists to beat.

BE LITERAL ABOUT WHAT THE PROJECT TOUCHES. If it is about stocks, the real companies belong in the frame by name and by mark. If it routes between chains, those chains are real and nameable. If it holds treasuries, that is real money and real institutions. Naming the actual things is what separates a banner made for THIS project from a technical-looking surface that would suit anything — and it is almost always the strongest move available to you.
...
A second habit, and the more dangerous one because it arrives disguised as polish: rendering the type as a material. The project name is set in flat, clean letterforms — real typography, the way it leaves a studio. No metallic, chrome, gold or brushed-steel fills. No bevel, emboss, inner glow, drop shadow or outer stroke. No extruded three-dimensional letters standing off the surface or receding into perspective. No gradient poured down the type, no specular highlights, no reflections in the letterforms. Depth belongs to what sits behind the name — to what is photographed, built, printed or arranged — never to the letters themselves. Type wearing a machined-metal finish is the fastest way to look like a 2009 Photoshop tutorial, and it is the first thing a client with taste will notice. Metal in the ARTWORK is welcome when the project genuinely points at it and it is a real material honestly lit; a gold gradient poured over a wordmark is not the same thing and is never the answer. If the client has explicitly asked for metallic or dimensional lettering, give it to them — otherwise the type stays flat.`,
    // ══ WHAT THE RENDERER STILL NEEDS AFTER THE DIRECTOR HAS DECIDED ══
    //
    // Under leanBrief the image model is not sent `mood`. It is sent
    // this. The difference is the difference between deciding and
    // executing: everything cut from here is about WHAT TO MAKE — how
    // to read a brief, what to reject, what a good idea looks like —
    // and by the time the renderer runs, that has been decided and
    // written down by the director.
    //
    // These three survive because they are the only ones the director
    // cannot enforce from a text concept. The quality bar, the name
    // being readable at a glance, and the ban on rendering type as a
    // material — chrome, bevel, extrusion — which arrives disguised as
    // polish and which no concept ever asks for.
    //
    // Lifted verbatim out of `mood`, and a test asserts each still
    // appears there, so the two cannot drift apart.
    render: `This is brand-grade design work — the standard a serious studio delivers to a client with taste.
...
export const PROMPT_FIELDS = [
  ...
  // Both halves of leanBrief. The flag alone would leave `render`
  // behind, and a Default run inheriting another style's rendering
  // rules is exactly the leak this list exists to stop.
  "leanBrief", "render",
];
...
// ══ WHAT THE DIRECTOR IS TOLD, WHEN IT IS WRITING THE ONLY BRIEF ══
//
// Under leanBrief the renderer never sees the project description or
// the reasoning behind the style — just the concept, the name and the
// mark. So the concept has to stop being a sketch and become the
// whole instruction.
//
// The non-negotiables are here for a reason found by testing rather
// than guessed at. The first lean renders came back with the mark
// drawn twice, a dropped tagline and a name set in lowercase — and
// none of those rules had been removed from the image prompt. The
// CONCEPT was simply outranking them. A director that does not know
// the name is lettered exactly as given will cheerfully specify
// something better, and the renderer will believe it over the rules.
//
// So the director is told the things it must design WITHIN. Not to
// constrain the idea — to stop it writing a brief that contradicts
// the frame it will be drawn in.
const OWNS_THE_BRIEF = `

YOU ARE WRITING THE ONLY BRIEF THE RENDERER WILL GET. It will not see this
project's description and it will not see the thinking above — only your
words, the client's mark, and the name to letter. So decide everything and
say it plainly, as continuous prose rather than a list: what fills the
frame and what it is made of; where the eye goes first; the palette in
actual colours; how the project name is set and where it sits; where the
client's mark goes and at what size; and what the whole thing should feel
like at a glance on a phone.

Be specific enough that two different designers would produce nearly the
same banner from it. Around 160 words. No preamble and no explanation of
your reasoning — the brief only.`;

// ══ AND THESE GO LAST, WHICH IS THE ONLY REASON THEY WORK ══
//
// They were part of the block above, in the middle of the director's
// message, with the whole palette after them. Both got ignored on the
// first real run: the mark was specified as "a recurring motif" and
// drawn eight times, and a name given as "Honey" came back as "HONEY".
//
// Nothing was wrong with the wording. It was in the wrong place —
// a ban buried mid-prose loses to everything printed after it, which
// is the oldest lesson in this file. Emitted last now, after the
// palette and immediately before the output instruction.
// Three rules, and there were four. The fourth said the name had to
// keep the exact capitalisation it was given, and the director ignored
// it on every run — a brief for "Honey" came back specifying uppercase
// each time. Looking at the render settled it: HONEY set large and
// flat is a normal typographic decision and it looked right. The rule
// was fighting good practice to prevent nothing.
//
// So it is gone, because a rule that is present and ignored is not
// free. Everything printed here competes for weight with everything
// else, and one instruction the model has already learned to step over
// makes the three beside it easier to step over too. Spelling still
// holds; case is the designer's call.
const DIRECTOR_LIMITS = `- DESIGN INSIDE THESE. They are not yours to improve on, and a brief that contradicts one is a brief the renderer will follow into a mistake. The project name is spelled EXACTLY as given — never abbreviated, never reworded, never translated. The client's mark appears ONCE, in one place, at one size: not repeated, not tiled, not scattered, and never "as a recurring motif". Any supporting line you were given is set, not dropped. Nothing — no letter, no mark — comes within 80 pixels of the left or right edge, or 45 of the top or bottom.`;

/** The style brief for the art director, which is not what the renderer gets. */
export function directorBrief(template) {
  if (!template?.mood) return "";
  return template.leanBrief ? template.mood + OWNS_THE_BRIEF : template.mood;
}

/** Hard constraints for a lean director, emitted last. Empty otherwise. */
export function directorLimits(template) {
  return template?.leanBrief ? DIRECTOR_LIMITS : "";
}
...
export function buildPrompt(template, brief, variantSeasoning = "", { auto = false, settings = {}, concept = "" } = {}) {
  ...
  // ══ leanBrief: THE RENDERER IS NOT SENT THE THINKING ══
  //
  // `render` instead of `mood`. What is cut is every paragraph about
  // WHAT TO MAKE — how to read a brief, what to reject, what a good
  // idea looks like — because under this flag the director has already
  // done that and written the answer into the concept below. Sending
  // both asks the renderer to re-decide something already decided,
  // with less to decide it from.
  //
  // Falls back to `mood` if a lean style has no render block, so the
  // flag can never silently strip a style down to nothing.
  const lean = !!template?.leanBrief && !auto;
  parts.push(
    auto
      ...
      : `Category mood for this piece: ${lean ? template.render || template.mood : template.mood}`
  );
...
LEGIBILITY FLOOR. Every word you do set must be readable at a glance. These banners are seen small — a strip a few centimetres wide on a phone, scrolled past in a second. Supporting lines sit below the name in HIERARCHY, which is a matter of scale, weight and placement — not a licence to set them so fine, so faint, so tightly tracked or so low in contrast that the viewer has to work at them. A subordinate line should still be read without effort. If a line is not worth setting at a readable size, leave it out altogether.`
  ...
  // Under leanBrief the briefing goes too. It is the raw material the
  // director worked FROM, and it is the loudest instruction in the
  // prompt — "THIS IS WHERE THE IDEA COMES FROM" — set against a
  // concept that already came from it. Leaving both in hands the
  // renderer the question and the answer and lets it choose.
  //
  // The name and the mark still arrive. Those are things to draw, not
  // things to interpret.
  parts.push(
    lean
      ? ""
...
Interpret it intelligently rather than literally: they are describing an outcome, not writing a specification, and they may not have the vocabulary for what they mean. If it is brief, treat it as the seed of the concept and build outward with your own judgment rather than doing only the one thing named. It does not override the canvas, framing or legibility requirements below — those are technical, not creative.`
  ...
  // filter(Boolean): a leanBrief run pushes an empty string where the
  // briefing would be, and joining that leaves a gap wide enough to

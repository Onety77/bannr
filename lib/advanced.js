...
export const PER_STYLE = {
  ...
  pov: [
    {
      key: "shot",
      // ══ THE ONE CONTROL YOU CAN GIVE SEVERAL ANSWERS TO ══
      //
      // Every other control asks what the whole run should look like,
      // so one answer is the right shape for it. This one asks where
      // the camera stands, and the honest answer is usually "show me
      // a few". Forcing that into a single choice meant paying for a
      // run to see one shot, which is the one thing the style is for.
      //
      // Picks spread one per option, in the order chosen, using the
      // same helper that spreads styles — so a run of three with three
      // shots picked returns exactly those three.
      multi: true,
      label: "Shot",
      help: "Pick as many as you have options — each banner takes a different one.",
...
export function isDefault(control, value) {
  if (control.multi) return !picked(value).length;
...
export function optionLabels(styleId, settings = {}, total = 1) {
  return Array.from({ length: Math.max(total, 1) }, (_, i) => {
    ...
    return controlsFor(styleId)
      .filter((c) => c.multi && picked(settings[c.key]).length > 1)
...
export function multiKeys(styleId) {
  return controlsFor(styleId).filter((c) => c.multi).map((c) => c.key);
}

// The same settings with every per-option control cleared — what
// applies to the STYLE as a whole rather than to one banner. The
// concept pass needs this split: a run asking for three different
// shots must not tell the director that all three concepts are the
// first one, which is exactly what a single buildDirection call on an
// unresolved multi value would do.
export function sharedSettings(styleId, settings = {}) {
  const out = { ...settings };
  for (const k of multiKeys(styleId)) out[k] = "auto";
  return out;
}

// The prose for the per-option controls alone, bare — no header,
// because it lands inside a numbered assignment list rather than
// standing on its own.
export function optionDirection(styleId, resolved = {}) {
  const out = [];
  for (const c of controlsFor(styleId)) {
    if (!c.multi) continue;
...
export function buildDirection(styleId, settings = {}) {
  ...
  for (const c of controlsFor(styleId)) {
    ...
    if (c.type === "scale") {
      ...
      const one = c.multi ? picked(v)[0] : v;

...
export async function saveImage(dataUrl, filename) {
  ...
  let blob;
  try {
    // ══ A DATA URL OR A PATH ══
    //
    // Everything used to be a data URL, because a banner only existed
    // in the tab that made it. My banners now re-serves the archived
    // file from /api/archive/{id}, so this takes a path too.
    //
    // SAME-ORIGIN ONLY, and that is not a limitation to route around.
    // A cross-origin fetch of a storage URL is refused by CORS unless
    // the bucket is configured for it, which is a setup step that
    // would have to be right on every environment — serving through
    // our own origin is why this works with none.
    blob = dataUrl.startsWith("data:")
      ? dataUrlToBlob(dataUrl)
      : await (await fetch(dataUrl, { credentials: "same-origin" })).blob();
    if (!blob || blob.size === 0) return { error: "That file couldn't be found." };
  } catch {

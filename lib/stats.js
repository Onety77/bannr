...
// Every banner ever made, for the public proof line.
//
// CUMULATIVE, not daily, and that is the whole reason it is a
// separate function. A daily figure is volatile — it can read 4 on a
// slow Tuesday and argue against us on the one page meant to
// convince someone — while a lifetime total only ever goes up. Same
// data, no bad days.
//
// One document per day, so this reads a few hundred rows a year and
// needs no index.
export async function madeTotal() {
  const db = getAdminDb();
  if (!db) return 0;
  try {
    const snap = await db.collection("stats").get();
    let n = 0;
    snap.docs.forEach((d) => { n += Number(d.data()?.generated || 0); });
    return n;
  } catch {
    return 0;
  }

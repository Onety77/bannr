...
// page load.
//
// KNOWN LIMIT: capped at 1000 payments, no pagination. That is a long
// way off and the cap is deliberate — an unbounded read on every load
// of a public page is how a launch takes the database down. When it
// starts to bind, this needs a running total written at payment time,
// not a bigger limit.
export async function productRevenue() {
  const db = getAdminDb();
  if (!db) return { sol: 0, usd: 0, count: 0 };
  try {
    ...
    snap.docs.forEach((d) => {
      ...
      // Only payments taken since packs moved to dollars carry `usd`.
      // Deliberately NOT reconstructed from today's rate for the rest:
      // the dollar value of a payment is what it was worth when it was
      // made, and inventing it later would make the published figure
      // move every time SOL did.
      usd += Number(p.usd ?? 0);
    });
    return {
      sol: Math.round(sol * 1e6) / 1e6,
    ...
    return { sol: 0, usd: 0, count: 0 };
  }
}

// SOL spent on buybacks, per source, across the WHOLE ledger.
//
// Separate from ledger()'s totals, which are the totals of the page
// being displayed and stop at whatever limit that page asked for.
// Using those to judge whether a commitment has been kept would show
// the promise as unmet the moment the ledger grew past one screen.
export async function spentBySource() {
  const db = getAdminDb();
  const out = { product: 0, fees: 0 };
  if (!db) return out;
  try {
    const snap = await db.collection("buybacks").limit(5000).get();
    snap.docs.forEach((d) => {
      const e = d.data() || {};
      const k = SOURCES.includes(e.source) ? e.source : "product";
      out[k] += Number(e.sol || 0);
    });
  } catch {}
  out.product = Math.round(out.product * 1e6) / 1e6;
  out.fees = Math.round(out.fees * 1e6) / 1e6;
  return out;
}

// ============================================================
// THE COMMITMENT, AND WHETHER IT IS BEING KEPT.
//
// "30% of revenue buys the token back" is the whole trader pitch, and
// a pitch is worth nothing without the arithmetic beside it. This is
// that arithmetic: what was earned, what that obliges, what has
// actually been spent, and the gap.
//
// ══ WHY THE GAP IS PUBLISHED EVEN WHEN IT IS NEGATIVE ══
//
// Showing only the total spent lets a promise quietly rot: the number
// goes up, slowly, and nobody can tell it is behind. The outstanding
// figure is the one that makes the commitment falsifiable, and a
// commitment that cannot be falsified is not a commitment. It is also
// the number that gets us to do the buyback.
//
// ══ AND WHY FEES ARE NOT IN IT ══
//
// The commitment is on PRODUCT revenue — customers paying for a thing.
// Fee buybacks are circular, traders discount them correctly, and
// counting them toward this would let a busy trading week discharge a
// promise made about a different kind of money entirely.
// ============================================================
// ══ AND WHY IT CANNOT TAX THE SAME SOL TWICE ══
//
// The obvious implementation tracks "revenue since the last buyback"
// and takes a cut of that. It is the one to avoid: it needs a marker
// moved on every buyback, and anything that puts the marker in the
// wrong place — a buyback logged late, two logged out of order, one
// deleted and re-added — silently re-taxes or skips a stretch of
// revenue, and nothing on the page would look wrong.
//
// This holds no marker. Both sides are recomputed from the totals:
//
//   owed        = pct × ALL revenue, ever
//   spent       = ALL product buybacks, ever
//   outstanding = owed − spent
//
// So each SOL of revenue contributes to `owed` exactly once and stays
// there, and every buyback reduces the gap permanently. Log them in
// any order, backdate one, delete one: the answer is the same,
// because the answer is a subtraction rather than a running tally.
export async function commitment(pct, everySol = 0) {
  const share = Math.min(Math.max(Number(pct) || 0, 0), 100);
  const [revenue, spent] = await Promise.all([productRevenue(), spentBySource()]);
  const owed = Math.round((revenue.sol * share) / 100 * 1e6) / 1e6;
  const done = spent.product;
  const outstanding = Math.max(0, Math.round((owed - done) * 1e6) / 1e6);

  // The trigger, in owed-SOL, derived from the revenue figure the
  // admin actually typed: "every 1 SOL earned" at 30% is due at 0.3.
  const every = Number(everySol) > 0 ? Number(everySol) : 0;
  const nudgeAt = every > 0 ? Math.round((every * share) / 100 * 1e6) / 1e6 : 0;

  return {
    pct: share,
    revenueSol: revenue.sol,
    revenueUsd: revenue.usd,
    purchases: revenue.count,
    owedSol: owed,
    spentSol: done,
    // One of these is always 0. Two names rather than a signed number
    // because the callers render them as different sentences, and
    // "-0.4 SOL outstanding" is a sentence nobody parses correctly.
    outstandingSol: outstanding,
    aheadSol: Math.max(0, Math.round((done - owed) * 1e6) / 1e6),
    // What was typed, and what it works out to.
    everySol: every,
    nudgeAt,
    // Time is not in this. A buyback is due when enough has been
    // earned to be worth swapping — a calendar would fire on a quiet
    // week with nothing to buy and stay silent through a busy Tuesday.
    due: nudgeAt > 0 && outstanding >= nudgeAt,
  };
}

// What the last seven days bought, which is the figure to publish.
//
// A percentage with no denominator invites people to imagine a large
// number and then feel lied to. A dollar amount, weekly, is the claim
// that can be checked — and it is the one that reads as a standing bid
// rather than a policy.
export async function lastSevenDays(entries) {
  const rows = entries || (await ledger(200)).entries;
  const since = Date.now() - 7 * 24 * 3600_000;
  const out = { sol: 0, bought: 0, burned: 0, count: 0, since };
  for (const e of rows) {
    if (!e?.ts || e.ts < since) continue;
    if (e.source !== "product") continue;
    out.sol += Number(e.sol || 0);
    out.bought += Number(e.bought || 0);
    out.burned += Number(e.burned || 0);
    out.count += 1;
  }
  out.sol = Math.round(out.sol * 1e6) / 1e6;
  return out;
}

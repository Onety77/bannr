...
// to choose two: the careful person paid exactly what the person
// taking everything paid.
//
// Each option is a separate image and a separate charge to us, so the
// price follows the count. Two costs two. Three and four both cost
// three, because the fourth is where the real spread comes from and
// pricing it separately would push everyone down to three — which
// costs us nearly as much and gives them less.
//
// HERE rather than in lib/users.js, which is server-only, because the
// create page has to show this number on the control that sets it. A
// price discovered afterwards on the balance is not a price anyone can
// choose against. A test pins it to GENERATION_COST so the two cannot
// drift.
export function runCost(variants) {
  return Number(variants) <= 2 ? 2 : 3;
}

// The most options a run paid from the free daily allowance may make.
// Paid runs are untouched.
//
// Every free run is four images of real money against no revenue, and
// at four a day the daily allowances were the largest single cost in
// the product. Three still shows a spread worth choosing between; the
// fourth is what turns a promotion into a bill.
export const FREE_RUN_MAX_OPTIONS = 3;

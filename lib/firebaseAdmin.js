...
export function getAdminBucket() {
  ...
  _bucketTried = true;
  const app = getAdminApp();
  // The Firebase console DISPLAYS the bucket as `gs://name`, and that
  // is the string anyone will copy. The SDK wants the bare name, and
  // rejects the prefixed one — so a perfectly reasonable paste would
  // silently leave the archive off with no error anywhere. Stripped
  // here rather than being a thing to remember.
  const name = String(process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET || "")
    .trim()
    .replace(/^gs:\/\//i, "")
    .replace(/\/+$/, "");
